...
const target = `      const data = await res.json();
                ...
                <div>
                  <button style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid var(--color-primary)', background: 'transparent', color: 'var(--color-primary)', fontWeight: 600, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-primary)'; e.currentTarget.style.color = '#111'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--color-primary)'; }}>
                    <Icon icon="solar:camera-bold-duotone" size={16} /> Cambiar Foto
                  </button>
                </div>
              </div>`;

const fix = `      const data = await res.json();
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: '24px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                  ...
                  <div>
                    <h2 style={{ margin: '0 0 4px 0', fontSize: '28px', fontWeight: 700 }}>{user?.name || 'Administrador'}</h2>
                    <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '15px' }}>{user?.role || 'Administrador del Sistema'}</p>
                  </div>
                </div>
                <div>
                  <button style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid var(--color-primary)', background: 'transparent', color: 'var(--color-primary)', fontWeight: 600, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-primary)'; e.currentTarget.style.color = '#111'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--color-primary)'; }}>
...
console.log('Fixed it really well');
